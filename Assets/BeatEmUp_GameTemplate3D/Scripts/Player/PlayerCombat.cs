...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject> {
	...
	public PostureManager postureManager;
	...
	void Start() {
		...
		GameObject uiObject = GameObject.Find("PostureUI"); // Replace "PostureUI" with the actual name
    	if (uiObject != null) {
        	postureManager = uiObject.GetComponent<PostureManager>();
    	}
	}
	...
	private void OnInputEvent(string action, BUTTONSTATE buttonState) {
		if (AttackStates.Contains (playerState.currentState) && !isDead) {
			...
			// Running punch when SpecialAttack is pressed
			if(action == "SpecialAttack" && buttonState == BUTTONSTATE.PRESS && isGrounded){
    			animator.SetAnimatorBool("Run", false);
    			if(RunningPunch.animTrigger.Length > 0) doAttack(RunningPunch, UNITSTATE.ATTACK, "Punch");
    			return;
			}
	...
	void Defend(DamageObject d){
		...
		// Modify Player's posture
		postureManager.AddPlayerPosture(2);  

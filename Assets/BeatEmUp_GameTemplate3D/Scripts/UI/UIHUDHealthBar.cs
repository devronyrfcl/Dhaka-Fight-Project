...
public class UIHUDHealthBar : MonoBehaviour {
    ...
    // New function for setting the enemy's portrait and name
    void SetEnemyPortraitAndName() {
        if (playerPortrait != null) {
            GameObject enemy = GameObject.FindGameObjectWithTag("Enemy");
            HealthSystem healthSystem = enemy.GetComponent<HealthSystem>();

            if (enemy && healthSystem != null) {
                // set portrait
                Sprite HUDPortrait = healthSystem.HUDPortrait;
                playerPortrait.overrideSprite = HUDPortrait;

                // set name
                nameField.text = enemy.GetComponent<EnemyActions>().enemyName;
            }
        }
    }
}
